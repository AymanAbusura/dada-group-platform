...
function OverviewSection({ data, kpis }) {
  ...
  // Compute beko and Ariston availability-based market share for KPI cards
  const { bekoShare, aristonShare } = useMemo(() => {
    const brands = availList.reduce((acc, item) => {
      const b = (item.brand || '').toLowerCase();
      if (!acc[b]) acc[b] = { yes: 0, total: 0 };
      acc[b].yes   += item.yes;
      acc[b].total += item.total;
    ...
    const beko    = brands['beko'];
    const ariston = brands['ariston'];
    return {
      bekoShare:    beko    && beko.total    > 0 ? Math.round((beko.yes    / beko.total)    * 100) : 0,
      aristonShare: ariston && ariston.total > 0 ? Math.round((ariston.yes / ariston.total) * 100) : 0,
    };
  }, [availList]);
    ...
    <>
      <div className="kpi-grid">
        {[
          ...
          { icon: '📋', val: kpis.total, lbl: 'تقرير مُرسل', accent: C.gold },
          { icon: '📦', val: kpis.models, lbl: 'موديل مرصود', accent: C.purple },
          { icon: '✅', val: `${kpis.availPct}%`, lbl: 'متوسط التوافر', accent: pctColor(kpis.availPct), color: pctColor(kpis.availPct) },
          { icon: '🟡', val: `${bekoShare}%`, lbl: 'حصة beko السوقية', accent: C.gold, color: pctColor(bekoShare) },
          { icon: '🔵', val: `${aristonShare}%`, lbl: 'حصة Ariston السوقية', accent: C.blue, color: pctColor(aristonShare) },
          { icon: '👥', val: kpis.reps, lbl: 'مندوب نشط', accent: C.teal },
          { icon: '🏆', val: kpis.topComp, lbl: 'أقوى منافس', accent: C.orange },
        ].map((k, i) => (
          <div key={i} className="kpi-card" style={{ '--accent': k.accent, animationDelay: `${i * 50}ms` }}>
...
function AvailSection({ data }) {
  ...
  return (
    <>
      {/* KPI Summary: show beko & Ariston aggregate stats + worst performing models */}
      <div className="kpi-grid" style={{ marginBottom: 14 }}>
        {(() => {
          const bekoAll    = availList.filter(i => (i.brand || '').toLowerCase() === 'beko');
          const aristonAll = availList.filter(i => (i.brand || '').toLowerCase() === 'ariston');
          const bekoAvg    = bekoAll.length    ? Math.round(bekoAll.reduce((s, i) => s + i.pct, 0)    / bekoAll.length)    : 0;
          const aristonAvg = aristonAll.length ? Math.round(aristonAll.reduce((s, i) => s + i.pct, 0) / aristonAll.length) : 0;
          // Pick 3 lowest-availability models as alerts
          const lowest = [...availList].sort((a, b) => a.pct - b.pct).slice(0, 3);
          return [
            { icon: '🟡', val: `${bekoAvg}%`,    lbl: 'متوسط توافر beko',    accent: C.gold,   color: pctColor(bekoAvg) },
            { icon: '🔵', val: `${aristonAvg}%`, lbl: 'متوسط توافر Ariston', accent: C.blue,   color: pctColor(aristonAvg) },
            ...lowest.map(item => ({
              icon:   (item.brand || '').toLowerCase() === 'ariston' ? '🔵' : '🟡',
              val:    `${item.pct}%`,
              lbl:    (item.label || item.model || '').substring(0, 18),
              accent: pctColor(item.pct),
            ...
            <div key={i} className="kpi-card" style={{ '--accent': k.accent, animationDelay: `${i * 60}ms` }}>
              <div className="kpi-icon">{k.icon}</div>
              <div className="kpi-val" style={{ color: k.color }}>{k.val}</div>
              <div className="kpi-lbl" title={k.lbl}>{k.lbl}</div>
            </div>
          ));
        })()}
      </div>
              ...
              {rec && (
                <div className="rec-box">
                  ...
                  <div>
                    ...
                    <div className="rec-text">
                      متوسط المنافسين: <strong>{rec.avg} JD</strong> — النطاق: <strong>{rec.mn}–{rec.mx} JD</strong>.{' '}
                      يُنصح بتسعير منتجك بين{' '}
                      <strong>{rec.mn + Math.round((rec.mx - rec.mn) * 0.30)} – {rec.mn + Math.round((rec.mx - rec.mn) * 0.65)} JD</strong>{' '}
                      لضمان تنافسية مع هامش ربح مناسب.
                    </div>
                  </div>
                </div>
              )}
              {!rec && modelDetail.competitors.length === 0 && (
                <div style={{ marginTop: 12, padding: '16px 20px', background: 'rgba(201,168,76,0.08)', border: `1px solid rgba(201,168,76,0.2)`, borderRadius: 10, textAlign: 'center' }}>
                  <div style={{ fontSize: 22, marginBottom: 6 }}>📭</div>
                  <div style={{ fontSize: 13, color: C.gold, fontWeight: 700, marginBottom: 4 }}>لا توجد أسعار منافسين مرصودة لهذا الموديل</div>
                  <div style={{ fontSize: 11, color: C.muted }}>اطلب من المندوبين تسجيل أسعار المنافسين عند الزيارة</div>
                </div>
              )}
            </>
          )}
          {selModel && !modelDetail && <EmptyState text="لا توجد بيانات لهذا الموديل" />}
        </div>
      </div>
    </>
  );
